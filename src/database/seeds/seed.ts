...
const prioritiesToCreate = async (app: INestApplicationContext) => {
    ...
    const priorityPromises = prioritiesToCreate.map((name) => priorityService.save({ name }));
...
const statusesToCreate = async (app: INestApplicationContext) => {
    ...
    const statusPromises = statusToCreate.map((name) => statusService.save({ name }));
...
const userToCreate = async (app: INestApplicationContext, roleId: string) => {
    ...
    if (!email || !password) throw new Error('ADMIN_EMAIL and ADMIN_PASSWORD variables are required')
